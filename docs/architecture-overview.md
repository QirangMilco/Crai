...
- 扩展加载
- 基础事件/钩子流水线
- 一个模型适配器（多个 provider 适配器可同时加载）
- 一个存储适配器
- 一个 Web UI 壳层（Vite + React + Tailwind + PWA）
- 提供默认行为的扩展组合（工具、安全、持久化）
- 配置系统（变体 + 全局 + 工作区，不依赖环境变量）
- 安全层（路径校验、敏感命令检测、三级权限模式、三层防御）
- 工具包分域拆分（fs、shell、web）
